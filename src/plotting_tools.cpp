...
ProgressFile::ProgressFile(const std::string& fullFileName, bool storeTuple)
    : filename_(fullFileName),
      storeTuple_(storeTuple)
{
    createDirectories(filename_);
}

void ProgressFile::addResult(const std::string& plannerName, const progress_pair_vector_t& progress)
{
    if (storeTuple_ == true)
    {
        throw ompl::Exception("Attempting to add a pair of data to a tuple progress file.");
    }
    //Variable:
    ...
    //Write the progress, time first:
    mfile << plannerName << ", ";
    for (unsigned int i = 0u; i < progress.size(); ++i)
    {
        mfile << std::setprecision(21) << progress.at(i).first.total_nanoseconds()/1e9;
        if (i != progress.size() - 1u)
        {
            mfile << ", ";
        }
    }
    ...
    for (unsigned int i = 0u; i < progress.size(); ++i)
    {
        mfile << std::setprecision(21) << progress.at(i).second;
        if (i != progress.size() - 1u)
        {
            mfile << ", ";
        }
...
void ProgressFile::addResult(const std::string& plannerName, const progress_tuple_vector_t& progress)
{
    if (storeTuple_ == false)
    {
        throw ompl::Exception("Attempting to add a tuple of data to a pair progress file.");
    }
    //Variable:
    //The output file stream
    std::ofstream mfile;
    ...
    for (unsigned int i = 0u; i < progress.size(); ++i)
    {
        mfile << std::setprecision(21) << progress.at(i).get<0u>().total_nanoseconds()/1e9;
        if (i != progress.size() - 1u)
        {
            mfile << ", ";
        }
    ...
    for (unsigned int i = 0u; i < progress.size(); ++i)
    {
        mfile << std::setprecision(21) << progress.at(i).get<1u>();
        if (i != progress.size() - 1u)
        {
            mfile << ", ";
        }
    ...
    for (unsigned int i = 0u; i < progress.size(); ++i)
    {
        mfile << std::setprecision(21) << progress.at(i).get<2u>();
        if (i != progress.size() - 1u)
        {
            mfile << ", ";
        }
...
TargetFile::TargetFile(const std::string& fullFileName)
    : filename_(fullFileName)
{
    createDirectories(filename_);
}

void TargetFile::addResult(const std::string& plannerName, const target_pair_vector_t& targetResults)
{
    //Variable:
    //The output file stream
    std::ofstream mfile;
    ...
    //Write the result, targets first:
    mfile << plannerName << ", ";
    for (unsigned int i = 0u; i < targetResults.size(); ++i)
    {
        mfile << std::setprecision(21) << targetResults.at(i).first;
        if (i != targetResults.size() - 1u)
        {
            mfile << ", ";
        }
    }
    ...
    for (unsigned int i = 0u; i < targetResults.size(); ++i)
    {
        mfile << std::setprecision(21) << targetResults.at(i).second.total_nanoseconds()/1e9;
        if (i != targetResults.size() - 1u)
        {
            mfile << ", ";
        }
...
IterationFile::IterationFile(const std::string& fullFileName)
    : filename_(fullFileName)
{
    createDirectories(filename_);
}

void IterationFile::addResult(const std::string& plannerName, const iteration_pair_vector_t& progress)
{
    //Variable:
    //The output file stream
    std::ofstream mfile;
    ...
    //Write the result, iteration first:
    mfile << plannerName << ", ";
    for (unsigned int i = 0u; i < progress.size(); ++i)
    {
        mfile << progress.at(i).first;
        if (i != progress.size() - 1u)
        {
            mfile << ", ";
        }
